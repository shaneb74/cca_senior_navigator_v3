...
from core.gcp_data import load_schema, evaluate
...
def generate_conversational_blurb(result, answers):
    ...
    try:
        ...
        blurb_parts = [
            base_messages.get(tier_name, f"Based on your assessment, we recommend {tier_name}.")
        ]
        ...
        if "Elopement / Exit-seeking" in behaviors:
            behavior_messages.append(
                "we've accounted for exit-seeking behaviors with secure environment features"
            )

        if behavior_messages:
            blurb_parts.append(
                f"Since you've mentioned certain behaviors, {', and '.join(behavior_messages)}."
            )
        ...
        if mobility in ["Uses wheelchair or scooter", "Bed-bound or limited mobility"]:
            blurb_parts.append(
                "Your mobility needs also suggest assistance with daily activities, which this option covers well."
            )
...
def render():
    ...
    ctx = get_user_ctx()
    _user_id = ctx["auth"].get("user_id", "guest")
    _product_key = "gcp"
    ...
    sections = [
        {"id": s["id"], "title": s["title"], "icon": section_icons.get(s["id"], "�")}
...
def render_section_questions(section_id):
    ...
    for question in section_data["questions"]:
        ...
        _required = question.get("required", False)
