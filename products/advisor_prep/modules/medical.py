"""
Advisor Prep - Medical Section

Integrates with Flag Manager for:
- Chronic conditions (Conditions Registry)
- Care flags (flag toggles)
- Auto-flag rules (≥1 condition → chronic_present, ≥2 → chronic_conditions)
"""
...
from products.advisor_prep.prefill import get_care_needs_prefill, update_care_need_flag
...
def render():
    ...
    if prefill_data and prefill_data.get("chronic_conditions"):
        st.info("💡 We've brought over some information from your Guided Care Plan and Cost Planner. Feel free to edit anything that's changed.")
    ...
    st.caption("Select any ongoing health conditions. This information helps your advisor recommend appropriate care options and resources.")
    
    # Get saved data from session or prefill
    saved_data = st.session_state["advisor_prep"]["data"]["medical"]
    current_conditions = saved_data.get("chronic_conditions") or prefill_data.get("chronic_conditions", [])
...
def _load_conditions_registry() -> list:
    ...
    registry_path = Path(__file__).parent.parent.parent.parent / "config" / "conditions" / "conditions.json"
...
def _save_medical_section(selected_conditions: list, selected_flags: list, medical_notes: str):
    ...
    care_flag_keys = ["memory_support", "mobility_limited", "behavioral_concerns", 
