...
def get_care_needs_prefill() -> dict[str, Any]:
    ...
    """
    result = {"chronic_conditions": [], "care_needs": {}}
    ...
    if any(flag in active_flags for flag in mobility_flags):
        ...
        result["care_needs"]["mobility_assistance"] = {
            "enabled": True,
            "reason": "From mobility assessment",
        }
    else:
    ...
    if "wound_care" in active_flags:
        result["care_needs"]["wound_care"] = {"enabled": True, "reason": "From your care plan"}
    else:
...
def update_care_need_flag(care_need_key: str, enabled: bool) -> None:
    ...
    if care_need_key in FLAG_MAPPINGS:
        ...
        if enabled:
            flag_manager.activate(
                flag_id, source="advisor_prep.medical", context=f"User enabled {care_need_key}"
            )
        else:
            flag_manager.deactivate(
                flag_id, source="advisor_prep.medical", context=f"User disabled {care_need_key}"
            )
...
__all__ = ["get_financial_prefill", "get_care_needs_prefill", "update_care_need_flag"]
