    ...
    try:
        ...
        if is_on_results_step:
            ...
            try:
                ...
                # Pipeline computes outcome at Daily Living - no eager derive needed
                # Only compute if user navigated directly to Results (edge case)
                gcp = st.session_state.get("gcp", {})
                if not gcp.get("summary_ready"):
                    print("[GCP_RENDER] Summary not ready - computing fallback outcome")
                    ...
                    print(f"[GCP_RENDER] Using pipeline result: tier={tier_pre}")

                if tier_pre:
                    mode = get_feature_gcp_mode()

                    # Summary already generated by pipeline at Daily Living
                    # Only call ensure_summary_ready if missing (edge case)
                    if not gcp.get("summary_ready"):
                        ensure_summary_ready(state_pre, flags_pre, tier_pre)
                        print(f"[GCP_RENDER] RESULTS summary processed for tier={tier_pre}")
                    else:
                        print(f"[GCP_RENDER] RESULTS summary from pipeline cache for tier={tier_pre}")
