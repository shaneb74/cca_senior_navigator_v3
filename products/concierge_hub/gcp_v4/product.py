...
def render():
    ...
    try:
        ...
        if is_on_results_step:
            ...
            print(f"[GCP_SUMMARY_READY] true - rendering with tier={gcp.get('published_tier')}")
            ...
            try:
                ...
                if not tier_pre:
                    ...
                    tier_pre = outcome.get("tier")
                
                print(f"[GCP_SUMMARY_BIND] tier={tier_pre} from published_tier")

                if not tier_pre:
                    st.warning("⚠️ We're preparing your recommendation… If this persists, click Continue on Daily Living again.")
                    print("[GCP_SUMMARY_BIND] No tier found - cannot render")
                    st.markdown("</div>", unsafe_allow_html=True)
                    return

                # Summary already generated by pipeline - no computation needed
                print(f"[GCP_RENDER] Using pipeline result: tier={tier_pre} summary_ready=True")
    ...
    # HOURS/DAY NUDGE: bottom banner removed — nudge now appears inline in Navi header
        ...
        # Use pipeline result from gcp.published_tier (computed at Daily Living)
        ...
        if outcome and not already_pub:
            # Use pipeline result instead of re-computing
            gcp = st.session_state.get("gcp", {})
            if gcp.get("published_tier"):
                # Build outcome dict from pipeline result
                gcp_outcome = {
                    "tier": gcp["published_tier"],
                    "allowed_tiers": gcp.get("allowed_tiers", []),
                    "flags": gcp.get("flags", []),
                    "advisory_meta": gcp.get("advisory_meta", {}),
                }
                _publish_to_mcip(gcp_outcome, module_state)
                _mark_published()
                print(f"[GCP_PUBLISH] Published pipeline result: tier={gcp['published_tier']}")
            else:
                ...
                try:
                    ...
                    _publish_to_mcip(gcp_outcome, module_state)
                    _mark_published()
                    print("[GCP_PUBLISH] Published fallback outcome (no pipeline result)")
                except Exception as e:
                    print(f"[GCP_PUBLISH_ERROR] {e}")
            ...
            try:
                ...
                # Don't fail if household handling fails
