...
def is_medicaid_unlocked(data):
    ...
    return (
        is_income_complete(data)
        and is_expenses_complete(data)
        and is_assets_complete(data)
        and is_care_complete(data)
    )
...
def calculate_monthly_cost(care_type, details, zip_code=None):
    ...
    multiplier = COST_CONFIG["zip_multipliers"].get(
        str(zip_code), COST_CONFIG["zip_multipliers"]["default"]
    )
...
def render_income():
    ...
    partner = st.radio(
        " ",
        ["Yes", "No"],
        index=0 if data.get("partner") else 1,
        key="partner_radio",
        label_visibility="collapsed",
    )
    _finance_handling = None
    if partner == "Yes":
...
def render_housing_options():
    ...
    if not gcp_exists:
        ...
        care_labels = [
            "In-Home Care (hourly, comes to you)",
            "Assisted Living (meals, help on site)",
            "Memory Care (secure, dementia-specialized)",
        ]
        default_index = (
            care_options.index(data.get("care_type", "in_home"))
            if data.get("care_type") in care_options
            else 0
        )
        ...
        if care_type == "in_home":
            hours = st.slider(
                "Daily Hours Needed: 0 ----[4]---- 24",
                1,
                24,
                data.get("care_details", {}).get("hours", 4),
            )
            details["hours"] = hours
        elif care_type == "assisted_living":
            ...
            intensity_labels = [
                "Standard ($7,500/mo)",
                "High Acuity (+$1,500/mo, ~$9,000/mo)",
            ]
            default_intensity = data.get("care_details", {}).get("care_intensity", "standard")
            intensity_index = (
                intensity_options.index(default_intensity)
                if default_intensity in intensity_options
                else 0
            )
    ...
    mobility_index = (
        mobility_options.index(default_mobility) if default_mobility in mobility_options else 0
    )
    mobility_label = st.radio(
        " ",
        mobility_labels,
...
def render_va_benefits():
    ...
    _finance_handling = data.get("finance_handling", "Joint household")
    ...
    if adl_flags:  # Has ADL needs
        if has_spouse and data.get("finance_handling", "Joint household") in [
            "Joint household",
            "Split household",
        ]:
    ...
    st.markdown(
        f"Based on your answers (needing help with {adl_text}) and veteran status, we estimate: **${estimated_benefit:,.0f}/month** ({benefit_type})."
    )
    ...
    if st.session_state["va_quiz_open"]:
        ...
        low_income = st.checkbox(
            "Household income under ~$25,000/year (veteran-alone)?", value=False
        )
        ...
        service_issue = st.checkbox(
            "Chronic issues (e.g., dementia, mobility) from service?", value=False
        )
        ...
        if eligible:
            quiz_benefit = (
                COST_CONFIG["va_benefits"]["veteran_spouse_aid_attendance"]
                if (
                    has_spouse
                    and data.get("finance_handling", "Joint household")
                    in ["Joint household", "Split household"]
                )
                else COST_CONFIG["va_benefits"]["veteran_alone_aid_attendance"]
            )
...
def render_expenses():
    ...
    utilities = st.number_input(
        "Utilities (Electric, Water, etc.)", value=data.get("utilities", 0), step=50
    )
    groceries = st.number_input("Groceries", value=data.get("groceries", 0), step=50)
    medications = st.number_input(
        "Medications/Healthcare", value=data.get("medications", 0), step=50
    )
    transportation = st.number_input(
        "Transportation (Car, Gas, Transit)",
        value=data.get("transportation", 0),
        step=50,
    )
    other_expenses = st.number_input(
        "Other Expenses",
        value=data.get("other_expenses", 0),
        step=50,
...
def render_assets():
    ...
    liquid_savings = st.number_input(
        "Liquid Savings (Checking/Savings)",
        value=data.get("liquid_savings", 0),
        step=1000,
    )
    retirement = st.number_input(
        "Retirement Accounts (401k, IRA)", value=data.get("retirement", 0), step=1000
    )

    homeowner = data.get("homeowner", False)
    home_equity = 0
    if homeowner:
        home_equity = st.number_input(
            "Home Equity (if owned)", value=data.get("home_equity", 0), step=10000
        )
...
def render_health_life_benefits():
    ...
    ltc = st.radio(
        " ",
        ["Yes", "No"],
        index=0 if data.get("long_term_care") else 1,
        key="ltc_radio",
        label_visibility="collapsed",
    )
    ltc_payout = 0
    ...
    medicare = st.radio(
        " ",
        ["Yes", "No"],
        index=0 if data.get("medicare") else 1,
        key="medicare_radio",
        label_visibility="collapsed",
    )
    ...
    whole_life = st.radio(
        " ",
        ["Yes", "No"],
        index=0 if data.get("whole_life") else 1,
        key="whole_life_radio",
        label_visibility="collapsed",
    )
    cash_value = 0
    expected_payout = 0
    ...
    if whole_life == "Yes":
        ...
        plan_to_sell = st.radio(
            " ",
            ["Yes", "No"],
            index=0 if data.get("plan_to_sell") else 1,
            key="sell_radio",
            label_visibility="collapsed",
        )
        if plan_to_sell == "Yes":
            expected_payout = st.number_input(
                "Expected one-time payout",
                value=data.get("expected_payout", cash_value),
                step=1000,
            )
...
def render_medicaid_navigation():
    ...
    if not is_medicaid_unlocked(data):
        ...
        st.image(
            "https://via.placeholder.com/50x50?text=🔒", width=50
        )  # Placeholder for padlock icon
    ...
    if already_on_medicaid:
        ...
        if st.session_state["spend_down_open"]:
            ...
            care_cost = calculate_monthly_cost(
                data.get("care_type", "in_home"), data.get("care_details", {})
            )
            income = sum(
                [data.get(k, 0) for k in ["social_security", "pension", "wages", "other_income"]]
            )
            expenses = sum(
                [
                    data.get(k, 0)
            ...
            if gap > 0:
                ...
                st.write(
                    f"With ${care_cost:,.0f}/mo care, ${income:,.0f}/mo income, ${gap:,.0f}/mo gap, ${assets:,.0f} assets, Medicaid covers in ~{months:.0f} months."
                )
            else:
                st.write("No spend-down needed - income covers care costs.")

            spend_down_amount = st.slider(
                "Spend before Medicaid?",
                0,
                min(assets, 154140),
                data.get("spend_down_amount", 0),
            )
