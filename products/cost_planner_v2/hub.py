...
def _render_summary():
    ...
    if modules_state.get("assets", {}).get("data"):
        ...
        with col3:
            real_estate_total = breakdown.get('primary_residence', 0.0) + breakdown.get('other_real_estate', 0.0)
            st.metric(
                "Real Estate", 
        ...
        if total_debt > 0:
            ...
            st.metric(
                "Net Asset Value",
                f"${net_assets:,.0f}",
                help="Assets available for care planning"
            )
...
def _publish_to_mcip():
    ...
    available_liquid_assets = max(
        asset_categories['liquid_assets'] - normalized_assets.get('liquid_assets_loan_balance', 0.0),
        0.0
    )
    legacy_asset_categories = {
        ...
        'investments': 0.0,
        'real_estate': asset_categories['primary_residence'] + asset_categories['other_real_estate'],
        'business': 0.0,
