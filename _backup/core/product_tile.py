...
# ----------------------------------------------------------------------------
class BaseTile:
    """Base functionality shared by hub product tiles and module tiles.

    Common concerns handled here:
      - gating/visibility/prereqs
      - progress derivation (0..100)
      - simple router hooks via query params (?go=...)
      - consistent HTML shells that rely on CSS in assets/css/dashboard.css
    """
    ...
    # ----- state helpers -----------------------------------------------------
    def _is_prereq_met(self) -> bool:
        if not self.prereq_key:
            return True
        state = st.session_state.get(self.prereq_key, {})
        return float(state.get("progress", 0)) >= 100.0

    def _status_tuple(self) -> Dict[str, Any]:
        if self.locked or not self._is_prereq_met():
            return {"state": "locked", "text": "Locked"}
        if self.progress >= 100:
            return {"state": "complete", "text": "Completed"}
        if self.progress > 0:
            return {"state": "in_progress", "text": "In Progress"}
        return {"state": "new", "text": "Not Started"}

    # ----- routing helpers ---------------------------------------------------
    @staticmethod
    def _href(go: str) -> str:
    ...
    # ----- extensibility hooks ----------------------------------------------
    def render(self) -> None:  # pragma: no cover - implemented by subclasses
        raise NotImplementedError


# ----------------------------------------------------------------------------
# Large Product tile for hubs
# ----------------------------------------------------------------------------
class ProductTileHub(BaseTile):
    """Large tile used on hub dashboards (blue gradient, status chip, two CTAs)."""
    ...
    def __init__(
        ...
        *,
        ...
        primary_go: Optional[str] = None,
        secondary_label: Optional[str] = None,
        secondary_go: Optional[str] = None,
        ...
        **kwargs: Any,
        ...
        self.primary_go = primary_go
        self.secondary_label = secondary_label
        self.secondary_go = secondary_go
    ...
    def render(self) -> None:
        ...
        head_html = (
            ...
            f'<h3 class="ptile__title">{html.escape(self.title)}</h3>'
            f'<span class="ptile__status">{html.escape(self.status_text or status["text"])}</span>'
            f"</div>"
        )
...
# ----------------------------------------------------------------------------
# Compact Module tile for inside products
# ----------------------------------------------------------------------------
class ModuleTileCompact(BaseTile):
    """Compact white card used for modules inside a product (badge, summary, links)."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        super().__init__(key=key, title=title, **kwargs)
        self.badge_text = badge_text
    ...
    def render(self) -> None:
        ...
        if self.primary_label:
            st.markdown(
                f'<a class="mtile__link" href="{self._href(self.primary_go)}">{self.primary_label}</a>',
                unsafe_allow_html=True,
            )
        if self.secondary_label:
            st.markdown(
                f'<a class="mtile__link" href="{self._href(self.secondary_go)}">{self.secondary_label}</a>',
                unsafe_allow_html=True,
            )
        # status
...
# ----------------------------------------------------------------------------
if __name__ == "__main__":
    st.set_page_config(page_title="Tile Demo", layout="wide")
    ...
    st.write("## Module Tiles")
    m1 = ModuleTileCompact(
        key="va",
        title="Veterans eligibility",
        badge_text="AI Assisted",
        summary_label="Status",
        summary_value="Not started",
        primary_label="Start",
        primary_go="cp_va_start",
        secondary_label="See responses",
        secondary_go="cp_va_view",
        progress=0,
    )
    m1.render()
