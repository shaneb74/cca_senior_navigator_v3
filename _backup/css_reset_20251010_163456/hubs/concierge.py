...
def render(ctx=None) -> None:
    ...
    cards = [
        ProductTileHub(
            key="gcp", title="Guided Care Plan", desc="Understand the situation",
            blurb="Personalized assessment across daily life, health, and cognition.",
            meta_lines=["≈12 minutes • Auto-saves"],
            primary_label="Start", primary_go="gcp_start",
            secondary_label="See responses", secondary_go="gcp_view",
            progress=gcp_prog, variant="brand", order=10, visible=True, locked=False
        ),
        ProductTileHub(
            key="cost", title="Cost Planner", desc="Estimate monthly costs and runway",
            ...
            primary_label="Start", primary_go="cost_open",
            secondary_label="See responses", secondary_go="cost_view",
            progress=cost_prog, variant="brand", order=20, visible=True, locked=False
        ),
        ProductTileHub(
            key="pfma", title="Plan with My Advisor", desc="Schedule a 1:1 planning session",
            blurb="Get matched with the right advisor to coordinate care, benefits, and trusted partners.",
            badge_text="CONCIERGE TEAM",
            primary_label="Get connected", primary_go="pfma_start",
            secondary_label="Share updates", secondary_go="pfma_updates",
            progress=0, variant="brand", order=30
        ),
        ProductTileHub(
            key="faqs", title="FAQs & Answers", desc="Ask the Senior Navigator AI",
            blurb="Instant, tailored assistance.", badge_text="AI AGENT",
            primary_label="Open", primary_go="faqs_open",
            progress=0, variant="teal", order=40
        ),
    ]
