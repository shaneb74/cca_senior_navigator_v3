...
class ModuleHub:
    ...
    def render(self) -> None:
        ...
        visible_modules = [m for m in self.modules if m.get("visible_if", lambda: True)()]

        if not visible_modules:
            st.warning(
                "⚠️ No modules available. Please update your profile or complete prerequisites."
            )
            return
    ...
    def get_visible_modules(self) -> list[dict[str, Any]]:
        ...
        """
        return [m for m in self.modules if m.get("visible_if", lambda: True)()]
