...
def render_partner_connection(partner: Dict[str, Any], ctx: Optional[Dict[str, Any]] = None) -> None:
    """
...
def _render_connection_form(partner: Dict[str, Any], config: Dict[str, Any], ctx: Optional[Dict[str, Any]]) -> None:
    ...
    with st.form(f"partner_connect_{partner_id}", clear_on_submit=False):
        ...
        consent = st.checkbox(
            "I consent to share my information with this partner",
            key=f"{partner_id}_consent"
        )
...
def _render_field(field_config: Dict[str, Any], partner_id: str) -> Any:
    ...
    if field_type == "text":
        ...
        return st.selectbox(label_display, options=[""] + options if not required else options, key=key)
...
def _submit_lead(partner: Dict[str, Any], fields: Dict[str, Any], submit_config: Dict[str, Any], ctx: Optional[Dict[str, Any]]) -> bool:
...
def _build_payload(template: Dict[str, Any], fields: Dict[str, Any], ctx: Optional[Dict[str, Any]], partner: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in template.items():
        if isinstance(value, str):
            ...
            payload[key] = [_interpolate_string(str(v), context_data) if isinstance(v, str) else v for v in value]
        else:
            payload[key] = value
...
def _log_lead_locally(partner_id: str, fields: Dict[str, Any], payload: Dict[str, Any], success: bool = True, error: Optional[str] = None) -> None:
