...
def on_recommendation_updated(payload: dict[str, Any]) -> None:
    ...
    try:
        ...
        log_event(
            "mcip.recommendation.viewed",
            {"tier": payload.get("tier"), "confidence": payload.get("confidence")},
        )
    except ImportError:
        pass
...
def on_financial_updated(payload: dict[str, Any]) -> None:
    ...
    try:
        ...
        log_event(
            "mcip.financial.completed",
            {"monthly_cost": payload.get("monthly_cost"), "confidence": payload.get("confidence")},
        )
    except ImportError:
        pass
...
def on_appointment_scheduled(payload: dict[str, Any]) -> None:
    ...
    try:
        ...
        log_event(
            "mcip.appointment.completed", {"date": payload.get("date"), "type": payload.get("type")}
        )
    except ImportError:
        pass
