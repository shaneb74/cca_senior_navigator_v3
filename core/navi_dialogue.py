...
class NaviDialogue:
    ...
    @classmethod
    def get_product_intro(cls, product_key: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def _format_message(cls, message: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in message.items():
            if isinstance(value, str):
                ...
                formatted[key] = [cls._format_string(v, context) if isinstance(v, str) else v for v in value]
...
def render_navi_message(
    ...
    # CTA button
    if show_cta and cta:
        if st.button(f"→ {cta}", key=f"navi_cta_{hash(cta)}", use_container_width=True, type="primary"):
            if cta_callback:
                cta_callback()
